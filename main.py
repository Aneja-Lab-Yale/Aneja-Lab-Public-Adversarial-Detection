...
from utils import get_acc_preds, plot_attacks_acc, plot_attacks_auc
...
acc1 = accuracy_score(y_test, (classifier.predict(x_test)>0.5))
print("Accuracy of original classifier on test set: %.2f%%, " % (acc1 * 100))
...
auc = roc_auc_score(y_train, (classifier.predict(x_train_adv)>0.5))
print("auc of original classifier on adversarial training set: %.2f%%, " % (acc * 100))
#
# Evaluate the classifier on the adversarial test examples
acc2 = accuracy_score(y_test, (classifier.predict(x_test_adv)>0.5))
print("accuracy of original classifier on adversarial test set: %.2f%%, " % (acc2 * 100))
auc2 = roc_auc_score(y_test, (classifier.predict(x_test_adv)>0.5))
print("auc of original classifier on adversarial test set: %.2f%%, " % (auc2 * 100))
